...
const args = document.location.search.substr(1).split('&').map(s => s.split('=')).reduce((p, c) => {
...
function summary(obj) {
  ...
  const body = Object.entries(obj.scans).map(([name, o]) => {
    o.name = name;
    return o;
  }).sort((a, b) => {
    if (a.detected && b.detected === false) {
      return -1;
    }
    if (a.detected === false && b.detected) {
      return 1;
    }
  }).map(scan => {
    return '<tr><td>' + scan.name + '</td><td class="' + (scan.detected ? 'defected' : 'clean') + '">' + scan.result + '</td><td>' + (scan.detail || '-') + '</td></tr>';
  }).join('');
...
chrome.runtime.onMessage.addListener(request => {
  if (request.cmd === 'report') {
    if (request.download) {
      ...
      document.querySelector('[data-id=time]').textContent = (new Date(request.download.startTime)).toLocaleString();
    }
    else {
      document.querySelector('[data-id=filename]').textContent = request.result.download?.filename || 'Unknown';
    }
    document.querySelector('[data-id=url]').textContent = request.url;
...
window.onblur = () => chrome.runtime.sendMessage({
  cmd: 'bring-to-front'
});
